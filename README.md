# ![@patrtorg/praesentium-cum-iure](logo-long.svg)
[![Version](https://img.shields.io/npm/v/@patrtorg/praesentium-cum-iure.svg)](https://www.npmjs.com/package/@patrtorg/praesentium-cum-iure)
[![CircleCI](https://circleci.com/gh/@patrtorg/praesentium-cum-iure/@patrtorg/praesentium-cum-iure/tree/main.svg?style=shield)](https://circleci.com/gh/@patrtorg/praesentium-cum-iure/@patrtorg/praesentium-cum-iure/tree/main)

[Try it now](https://@patrtorg/praesentium-cum-iure.org) |
[NPM](https://npmjs.com/package/@patrtorg/praesentium-cum-iure) |
[Github](https://github.com/@patrtorg/praesentium-cum-iure/@patrtorg/praesentium-cum-iure)
...
# Projects using @patrtorg/praesentium-cum-iure
...
```typescript
import { CallSite } from '@patrtorg/praesentium-cum-iure';
...
```typescript
import { reflect } from '@patrtorg/praesentium-cum-iure';
...
```
npm install @patrtorg/praesentium-cum-iure reflect-metadata
...
// tsconfig.json
"compilerOptions": {
    "plugins": [{ "transform": "@patrtorg/praesentium-cum-iure/dist/transformer" }]
}
```
...
@patrtorg/praesentium-cum-iure work just fine when using the official Typescript plugin. Using the `sucrase` plugin is not supported.

## **Jest**
See https://github.com/patrtorg/praesentium-cum-iure-jest for a sample repo with jest setup.
...
The `reflect()` API is the entry point to all types of reflection offered by `@patrtorg/praesentium-cum-iure`.
...
Accepting a function parameter marked with the `CallSite` type is how `@patrtorg/praesentium-cum-iure` knows that call site information
should be passed to the function. When making calls from one call-site enabled function to another, `@patrtorg/praesentium-cum-iure`
...
This works for all types that `@patrtorg/praesentium-cum-iure` can reflect, including unions, intersections, interfaces, classes,
...
Enabling `emitDecoratorMetadata` causes `@patrtorg/praesentium-cum-iure` to emit both the `design:*` style of metadata as well as its
own `rt:*` format. Disabling it causes only `rt:*` metadata to be emitted.

# Unsupported Scenarios

Some Typescript options are incompatible with `@patrtorg/praesentium-cum-iure`:
...
- `transpileOnly` -- For build solutions which provide it note that `transpileOnly` will not work correctly as
  `@patrtorg/praesentium-cum-iure` relies on Typescript's semantic analysis which is not performed when performing a direct
  transpilation.
...
`@patrtorg/praesentium-cum-iure`. We are [investigating](https://github.com/@patrtorg/praesentium-cum-iure/@patrtorg/praesentium-cum-iure/issues/19) how to enable
...
Ideally `@patrtorg/praesentium-cum-iure` should fail gracefully under these conditions, but for now it will help avoid duplicate issue
...
- [gfx/@patrtorg/praesentium-cum-iure](https://github.com/gfx/@patrtorg/praesentium-cum-iure) (No relation to this codebase)
